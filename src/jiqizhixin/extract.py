...
async def fetch_latest_articles(
    ...
    url = "https://www.jiqizhixin.com/api/article_library/articles.json?sort=time"
    p = None
    browser = None
    try:
        ...
        for article_data in articles_data[:limit]:
            ...
            task = asyncio.create_task(
                get_full_content(article_url, browser, semaphore, loop)
            )
            formatted_articles.append(
                {"title": title, "url": article_url, "task": task}
            )

        contents = await asyncio.gather(
            *(article.pop("task") for article in formatted_articles)
        )
